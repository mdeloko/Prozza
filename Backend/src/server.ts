...
ioServer.on("connection",(client)=>{    
    ...
    client.on("requestRoom",(sender:string,receiver:string,callback)=>{
        if(sender&&receiver){
            const room = [sender,receiver].sort().join("-");
            if(client.rooms.has(room)){
                callback(200,`Já estava na sala: ${room}`)
            }
            callback(200,room)
            client.join(room)
    ...
    client.on("message", async(userSent:string,userReceive:string,msg:string,room:string,callback)=>{
        try{
            ...
            if(usrSender&&usrReceiver){
                ...
                callback({status:200})
                client.to(room).emit("returnMessage",mg.content,mg.timeSent.getHours(),mg.timeSent.getMinutes());
            }
        }catch(err){
            console.error(err)
    ...
    client.on("loginTry",async (str:string,password:string,callback)=>{
        try{
    ...
    client.on("requestMessages", async (sender:string,receiver:string,callback)=>{
        try{
            ...
            if(chat){
                ...
                console.log(`[Backend - Server.ts] não pude enviar histórico de chat de (${sender}-${receiver}) para (${sender})!`);
                callback(404,null);
            }
        }catch(err){
    ...
    client.on("addFriend",async (sender:string,str:string)=>{
        try{
